...
def NavigateToFolder_Win11(browsefolderswindow, path, log_file=None):
    """Enhanced navigation function for Windows 11 with direct path entry"""
    from pywinauto.keyboard import send_keys
    from time import sleep as TimeSleep
    import os
    ...
    try:
        # First attempt: Direct path entry via address bar
        browsefolderswindow.set_focus()
        TimeSleep(0.5)
        
        # Try multiple keyboard shortcuts to access address bar
        shortcuts = ['%d', '^l', '{F4}']  # Alt+D, Ctrl+L, F4
        for shortcut in shortcuts:
            try:
                ...
                # Check if navigation worked by verifying OK button is enabled
                ok_button = browsefolderswindow.child_window(title="OK", class_name="Button")
                if ok_button.exists() and ok_button.is_enabled():
                    if log_file:
                        enhanced_logging("Direct path navigation successful", log_file)
                    return True
            except:
                continue
                
        # Second attempt: Look for edit control
        try:
            edit = browsefolderswindow.child_window(class_name="Edit")
            edit.set_text(path)
            TimeSleep(0.5)
            send_keys('{ENTER}')
            TimeSleep(1.5)
            ...
            if ok_button.exists() and ok_button.is_enabled():
                if log_file:
                    enhanced_logging("Edit control navigation successful", log_file)
                return True
        except:
            pass
            
        # Third attempt: Try to find folder in current view
        try:
            # Split path into components
            parts = path.split('\\')
            target_folder = parts[-1]  # Get the last folder name
            
            # Look for the target folder in visible elements
            all_elements = browsefolderswindow.descendants()
            for element in all_elements:
                try:
                    if element.window_text().strip() == target_folder:
                        element.click_input()
                        TimeSleep(0.5)
                        ...
                        if ok_button.exists() and ok_button.is_enabled():
                            if log_file:
                                enhanced_logging("Found target folder directly", log_file)
                            return True
                except:
                    continue
        except:
        ...
        if log_file:
            enhanced_logging(f"Navigation error: {str(e)}", log_file)
        return False
#############################################
# MAIN CODE
#############################################
...
if __name__ == "__main__":
    ...
    try:
        ...
        batPath = "C:\\P\\generate-data-sorting-key-file.bat"
        try:
